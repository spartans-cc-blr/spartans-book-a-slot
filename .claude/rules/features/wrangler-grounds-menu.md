...
## 6. Pitch Type field — added, then moved off grounds the same day (September 2026)

`grounds.pitch_type` briefly existed as a third classifiable ground
attribute alongside the maps/hospital links (migration
`078_grounds_pitch_type.sql`), specifically so Team Record could offer a
Pitch Type filter/split/then-by dimension, with edit access here following
the same wrangler/GC split §4 already established for the rest of this
page.

**Moved off `grounds` onto `tournaments` the same day** — a ground's
physical surface can change over time (resurfacing), which would silently
repaint every historical match's Team Record split once corrected; a
tournament runs on one surface for its whole duration, so it's the right
place to snapshot this instead. See `features/team-stats.md` §6 for the
full reasoning, the migration that moved it
(`079_move_pitch_type_to_tournaments.sql`), and where it lives now
(`/admin/tournaments`, admin-only — not this page). `GroundsClient.tsx`
and `src/app/api/grounds/route.ts` have no pitch-type UI or validation any
more — both are back to just Maps/Hospital, exactly as before this section
existed.
...
| `src/components/wrangler/GroundsClient.tsx` | Client table — add/edit forms, missing-link advisory; gated by `canAdd`/`canEdit` props. No pitch-type UI (moved off this page, §6) |
| `src/app/api/grounds/route.ts` | GET public; POST `isGC \|\| isAdmin`; PATCH `isWrangler \|\| isAdmin`. No pitch-type validation (moved to `/api/tournaments`, §6) |
| `supabase/migrations/078_grounds_pitch_type.sql` | Superseded the same day by `079_move_pitch_type_to_tournaments.sql` — see `features/team-stats.md` §6 |
