...
**Practice-tournament exclusion:** the *tournament* scope skips a booking whose tournament is the practice-games umbrella (`tournaments.is_practice`), additively OR'd with the booking's own `is_practice` override since September 2026 (`features/practice-games.md`) — same "real stats only" posture every other ranking surface in this app applies. The *ground* scope is unaffected (grounds aren't practice-specific).
