...
| `/players` | Server → `PlayerDirectoryGrid` (client) | Club-wide player directory (replaced the GC-only `/gc-players`, which now redirects) — public-profile `players` fields only (no wallet/status), last played from `squad`⋈`bookings`, career highlights from the analytics DB via `getCareerHighlightsByPlayer()`; every card links to `/players/[id]/stats` — see `features/player-directory.md` |
