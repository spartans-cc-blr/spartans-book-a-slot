...
**Status:** ✅ Route logic is correct and confirmed working. ⚠️ The underlying scheduling problem is bigger than this one route — see the resolution below and `limitations.md`'s "Vercel Hobby — Cron Jobs Do Not Reliably Fire" entry.
...
3. **The "wait and watch" Thursday (2026-07-16) confirmed it was still broken.** By 08:35 IST — 35 minutes past the scheduled 08:00 IST fire, and inside Vercel's own disclosed 1-hour flexible window but close to its edge — none of the weekend's 3 confirmed bookings were locked, and Supabase API logs spanning the fire time showed zero `PATCH` requests to `bookings`. Manually clicking "Run" on the route from the Vercel dashboard succeeded instantly (booking rows locked, GC push notifications sent), proving the route itself has never been the problem — only Vercel's own invocation of it.
...
**Resolution (2026-07-16):** the previously-deferred fix is now built and merged — a **GitHub Actions scheduled workflow** (`.github/workflows/cron-lock-availability.yml`) hits the same endpoint with the same `CRON_SECRET`, on a real `30 2 * * 4` weekly expression (GitHub Actions supports day-of-week restriction natively, so no in-code guard is needed on that side). The route is idempotent, so this runs safely alongside the still-present `vercel.json` entry. The same investigation found `/api/cron/backfill-scorecards` in an even worse state — **zero evidence of ever firing automatically**, ever, with a ~33-booking backlog quietly accumulated as a result — and `/api/cron/availability-nudge` intermittently missing days (a confirmed gap on 2026-07-14). All five project crons now have the same GitHub Actions backstop; see `architecture.md` §5 and `limitations.md` for the full writeup. Requires `CRON_SECRET` to be added as a GitHub repo secret (Settings → Secrets and variables → Actions) for the workflows to actually authenticate.

There's also a minor, non-urgent correctness smell in the route worth fixing whenever this is next touched: the Thursday-check uses an IST-shifted clock (`nowIST`) but the Saturday/Sunday date math a few lines below it uses the unshifted `now` — harmless at exactly 02:30 UTC firing time, but an inconsistency that shouldn't be there.
