...
//
// IMPORTANT — max first, resolve second (fixed 28 Jul 2026): totals are
...
// reconciliation (same stale-cache class as the incident already
// documented below in this file's history / post-match-scorecard.md §15),
// so pre-filtering silently dropped both of them and picked the highest
// score among whichever handful of players happened to have an exact
// squad-name match — a real, verifiably wrong "MVP". Resolving after the
// max (like computeTopPerformers()) means an unresolvable winner now
// correctly yields no share target rather than substituting a lesser
// candidate. Squad-only opponents are handled the same way this already
// worked for computeTopPerformers(): they simply resolve to player_id: null
// and produce no grantable/messageable target — never a fabricated winner.
