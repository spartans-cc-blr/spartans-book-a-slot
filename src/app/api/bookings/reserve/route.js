...
// Admin only. Creates a soft_block reservation with 48hr expiry.
export async function POST(req: NextRequest) {
  ...
  if (!game_date || !slot_time || !organiser_name) {
    return NextResponse.json({ error: 'Missing required fields: game_date, slot_time, organiser_name' }, { status: 400 })
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      notes:           notes ?? null,
      // Non-nullable fields — set to null for reservations
      format:          null,
      captain_id:      null,
      tournament_id:   null,
      venue:           null,
