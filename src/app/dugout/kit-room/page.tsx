...
export default async function KitRoomPage() {
  ...
  const [playerResult, ordersResult, settingsResult] = await Promise.all([
    ...
    supabase
      .from('jersey_orders')
      .select('id, jersey_name, jersey_number, jersey_size, jersey_half_sleeve_size, jersey_full_sleeve_size, tracks_size, jersey_name_override, jersey_number_override, jersey_half_sleeve_qty, jersey_full_sleeve_qty, tracks_qty, notes, status, created_at')
