...
export function LeaderboardFilters({ years, tournaments, grounds, year, tournamentId, groundId, formats, category }: Props) {
  ...
  // Milestones is a career-wide/top-level view, not meaningfully scoped to
  // a single tournament or ground — force both filters back to "all"
  // whenever this tab is selected (both selects are also disabled below
  // while it's active, so they can't be re-narrowed after the fact).
  // Format (T20/T30) is left enabled — "best economy in T20 games" is a
  // meaningful milestone in a way "...at this one tournament" isn't, the
  // same reasoning that already leaves Year enabled here.
  function selectCategory(c: LeaderboardCategory) {
    navigate(c === 'milestones' ? { category: c, tournament: 'all', ground: 'all' } : { category: c })
  }
  ...
  const milestonesActive = category === 'milestones'
    ...
    <div className="flex flex-col gap-2 mb-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <select
          ...
          disabled={milestonesActive}
          title={milestonesActive ? 'Milestones is a top-level view — not scoped to a tournament' : undefined}
        ...
        <select
          ...
          disabled={milestonesActive}
          title={milestonesActive ? 'Milestones is a top-level view — not scoped to a ground' : undefined}
          className={`form-input font-rajdhani text-xs py-1.5 disabled:opacity-50 disabled:cursor-not-allowed ${SELECT_TRUNCATE}`}>
